...
def stripe_webhook():
    ...
    if etype == "checkout.session.completed":
        ...
        meta        = (data.get("metadata") or {}) | (data.get("subscription_metadata") or {})
        ...
        item   = (sub["items"]["data"][0] if sub and sub["items"]["data"] else None)
        sub_item_id = item["id"] if item else None
        ...
        # opcional: avisar / degradar tras gracia
        pass

    return jsonify({"ok": True})

# -------- Opcional: webhook de Clerk (requires svix) --------------
try:
