...
def _init_stripe():
  """
  Inicializa Stripe usando el helper centralizado stripe_svc.init_stripe().
  Devuelve (sk, err) para compat con el código existente.
  """
  ...
  try:
    ...
    current_app.logger.exception("[Stripe] init error: %s", e)
    return None, (jsonify(error="STRIPE_SECRET_KEY invalid"), 500)
  return sk, None
...
def _resolve_entity(meta: dict, fallback: dict | None = None) -> dict:
  ...
  Nuevo (recomendado):
    ...
    - entity_id: id de Clerk (user u org)
    - entity_email
    - buyer_user_id
    - seats

  Flujo anterior (retrocompat):
    - scope: 'user' | 'org'
    - org_id / clerk_user_id / user_id
    - buyer_user_id
    - seats
  """
  md = dict(fallback or {})
  ...
  if not entity_type:
    ...
    if scope == "org":
      ...
      entity_id = (
        entity_id
        or (md.get("clerk_user_id") or md.get("user_id") or "").strip()
      )
...
def stripe_webhook_api():
  ...
  current_app.logger.info("[Stripe] webhook %s (%s)", event_id, etype)
  ...
  try:
    ...
    if etype == "checkout.session.completed":
      ...
      md = (sess.get("metadata") or {})  # entity_type/entity_id/... o scope/org_id/...
      ...
      seats = (
        _sum_seats_from_subscription(sub)
        if sub
        else int((md.get("seats") or 1))
      )
      ...
      # ───── Plan ENTERPRISE (org-level) ─────
      elif entity_type == "org" and entity_id:
        # Garantizar metadata consistente en el customer (entity_type/org, entity_id, entity_email)
        try:
          if customer_id:
            cust = stripe.Customer.retrieve(customer_id)
            md_cust = dict((cust.get("metadata") or {}) if isinstance(cust, dict) else {})
            changed = False
          ...
          current_app.logger.exception(
            "[Stripe] no se pudo garantizar metadata de customer"
          )
        ...
        try:
          ...
          current_app.logger.exception(
            "[Stripe] no se pudo promover buyer a admin"
          )

        # Propagar entitlement a miembros (enterprise_member) cuando esté activo
        try:
          ...
          current_app.logger.exception(
            "[Stripe] no se pudo propagar entitlement a miembros"
          )
    ...
    # ───────── customer.subscription.* ─────────
      ...
      # Org-level (ENTERPRISE)
        ...
        try:
          ...
          current_app.logger.exception(
            "[Stripe] no se pudo propagar entitlement a miembros (subs.*)"
          )
...
# NOTA: URLs de webhooks canónicas e inmutables según decisiones del proyecto:
#   - POST /api/stripe
#   - POST /api/clerk
