...
# ─────────────────────────── Estructuras vacías ───────────────────────────
_EMPTY_RESUMEN = {"summary": "", "key_changes": [], "key_dates_events": [], "conclusion": ""}
...
_DATE_PATTERNS = [re.compile(rf"\b(\d{{1,2}}\s+de\s+{_MONTHS}\s+de\s+\d{{4}})\b", re.I), re.compile(r"\b(\d{1,2}/\d{1,2}/\d{2,4})\b")]
...
def _json_schema_completion_with_retry(
    ...
    *,
    ...
    if fallback_to_json_object_on_timeout:
        # reduce tokens (máx. 350 o factor del original) para acelerar respuesta
...
def get_openai_responses(title: str, content: str) -> Tuple[str, str, str]:
    ...
    try:
        title_resp = _chat_completion_with_retry(client, messages=title_messages, model=_MODEL_TITLE, max_tokens=40, temperature=0.2, deadline_ts=deadline_ts, seed=7)
    ...
    for idx, ch in enumerate(chunks, start=1):
        resumen_messages = [
            {"role": "system", "content": "Eres un asistente legal experto en el BOE (España). Responde EXCLUSIVAMENTE en JSON válido conforme al esquema. No añadas texto fuera del JSON. No inventes datos."},
            {"role": "user", "content": "\n".join([
                ...
                "", "<<<CONTENIDO>>>", ch, "", "<<<PISTAS_DETECTADAS>>>", json.dumps(hints, ensure_ascii=False),
            ])},
        ]
        try:
    ...
    for idx, ch in enumerate(chunks, start=1):
        impacto_messages = [
            {"role": "system", "content": "Eres un analista legislativo. Responde EXCLUSIVAMENTE en JSON válido conforme al esquema. No añadas nada fuera del JSON. No inventes. Usa SOLO el CONTENIDO de esta parte."},
            {"role": "user", "content": "\n".join([
                f"(Parte {idx}/{len(chunks)}) Devuelve EXACTAMENTE este objeto con el esquema.",
                "Guía: CONVOCATORIA (afectados, cambios, riesgos, recomendaciones), LICITACIÓN (plazos, solvencia, garantías), RESOLUCIÓN (obligaciones/efectos).",
