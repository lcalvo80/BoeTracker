    ...
    _ENRICH_CONNECT_TIMEOUT = float(os.getenv("ENRICH_CONNECT_TIMEOUT", "12"))  # más largo que 10
    ...
    _ENRICH_MIN_SLEEP = float(os.getenv("ENRICH_MIN_SLEEP", "0.10"))  # 100ms
    _ENRICH_MAX_SLEEP = float(os.getenv("ENRICH_MAX_SLEEP", "0.30"))  # 300ms
    ...
    # cache ligera de proceso (evita re-golpear el mismo id)
    _CACHE: dict[str, str] = {}


    def _sleep_jitter():
        import random
        delay = random.uniform(_ENRICH_MIN_SLEEP, _ENRICH_MAX_SLEEP)
        time.sleep(delay)
    ...
    def _fetch_text_url(url: str) -> Optional[str]:
        ...
        if 400 <= resp.status_code < 600:
            logging.warning(f"enricher: HTTP {resp.status_code} en {url}")
            return None
    ...
    def _pdf_to_text(url_pdf: str) -> Optional[str]:
        if not url_pdf or pdf_extract_text is None:
            return None
        _sleep_jitter()
        # descargamos en streaming a memoria (peinando tamaño)
        try:
            with _session.get(url_pdf, headers={"User-Agent": _ENRICH_USER_AGENT}, timeout=(_ENRICH_CONNECT_TIMEOUT, _ENRICH_READ_TIMEOUT), stream=True) as r:
                if 400 <= r.status_code < 600:
                    logging.warning(f"enricher: PDF HTTP {r.status_code} en {url_pdf}")
                    return None
                content = r.content
            # extraer texto
            import io
            ...
            logging.warning(f"enricher: fallo extrayendo PDF {url_pdf}: {e}")
            return None
    ...
    def enrich_boe_text(
        ...
        base_text: str,
        min_gain_chars: int = 800,
    ) -> Tuple[str, bool]:
        """
        Devuelve (texto_enriquecido, enriched_bool).
        Estrategia:
        1) TXT (txt.php?id=...)
        2) DOC/HTML (doc.php?id=... o el url_html que venga)
        3) PDF → texto
        Solo considera 'enriquecido' si se gana >= min_gain_chars.
        """
        try:
            ...
            # 2) DOC HTML variantes
            if bid:
                candidates.append(f"https://www.boe.es/buscar/doc.php?id={bid}")
                candidates.append(f"https://www.boe.es/diario_boe/mostrar_datos.php?id={bid}")  # otra variante pública

            # 3) si vino un url_html directo del feed, considerarlo
            ...
            for url in candidates:
                try:
                    ...
                    logging.warning(f"enricher: ConnectTimeout en {url}")
                    t = None
                except requests.exceptions.ReadTimeout:
                    logging.warning(f"enricher: ReadTimeout en {url}")
                    t = None
                except requests.exceptions.RequestException as e:
                    logging.warning(f"enricher: error de red en {url}: {e}")
                    t = None

                if t and len(t) > base_len + min_gain_chars:
                    best = t
                    _CACHE[identificador] = t
                    logging.info(f"🧩 Enriquecido con HTML/TXT ({identificador}) +{len(t) - base_len} chars")
                    return best, True

            # 4) PDF fallback
            if url_pdf:
                pdf_text = _pdf_to_text(url_pdf)
                if pdf_text and len(pdf_text) > base_len + min_gain_chars:
                    _CACHE[identificador] = pdf_text
                    logging.info(f"🧩 Enriquecido desde PDF ({identificador}) +{len(pdf_text) - base_len} chars")
                    return pdf_text, True

            return best, best != (base_text or "")

        except Exception as e:
            logging.warning(f"enricher: fallo general {identificador}: {e}")
            return base_text, False
